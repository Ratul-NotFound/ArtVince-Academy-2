            ...
            {isOpen && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-6">
                    ...
                    <motion.div
                        ...
                        className="relative bg-surface border border-border w-full max-w-xl rounded-[40px] shadow-2xl overflow-hidden flex flex-col max-h-[90vh]"
                    >
                        {/* Header */}
                        <div className="p-8 border-b border-border flex justify-between items-center bg-foreground/[0.02]">
                            <div>
                                <h2 className="font-robot text-sm uppercase tracking-[0.3em] text-primary font-bold mb-1">Secure Enrollment</h2>
                                <p className="font-robot text-2xl font-bold uppercase tracking-tighter text-foreground truncate max-w-[300px]">
                                    {course.title}
                                </p>
                            </div>
                                ...
                                <div className="space-y-8">
                                    ...
                                    {step === 1 ? (
                                        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
                                            <h3 className="font-robot text-lg font-bold uppercase mb-6">Select Payment Method</h3>
                                            <div className="grid grid-cols-3 gap-4 mb-8">
                                                {(["bKash", "Nagad", "Rocket"] as const).map((m) => (
                                                    <button
                                                        key={m}
                                                        onClick={() => setMethod(m)}
                                                        className={`p-4 rounded-2xl border-2 transition-all flex flex-col items-center gap-2 ${method === m ? "border-primary bg-primary/5" : "border-border hover:border-primary/30"
                                                            }`}
                                                    >
                                                        <div className="w-10 h-10 rounded-xl bg-foreground/5 flex items-center justify-center font-robot font-black text-[10px]">
                                                            {m[0]}
                                                        </div>
                                                        <span className="font-robot text-[10px] uppercase tracking-widest font-bold">{m}</span>
                                                    </button>
                                                ))}
                                            </div>
